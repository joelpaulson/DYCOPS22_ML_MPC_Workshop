...
% predict confidence bounds on the successor state from random values
beta = 2; % confidence bound
Ntest = 25; % number of test points
xu_min = [x_min , u_min];
xu_max = [x_max , u_max];
XU_test = (xu_max - xu_min).*rand(Ntest, length(xu_min)) + xu_min;
X1_next_true = zeros(Ntest,1);
X1_next_lcb = zeros(Ntest,1);
X1_next_ucb = zeros(Ntest,1);
Within_Bounds = nan*ones(Ntest,1);
for i = 1:Ntest
    x_test = XU_test(i,1:2)';
    u_test = XU_test(i,3)';
    [w_test_mean, w_test_std] = predict(gp, [x_test ; u_test]');
    x_next_mean = A*x_test + B*u_test + Bd*w_test_mean;
    x_next_cov = Bd*w_test_std^2*Bd';
    x_next_true = plant(x_test,u_test,0);

    X1_next_true(i) = x_next_true(1);    
    X1_next_lcb(i) = x_next_mean(1) - beta*sqrt(x_next_cov(1,1));
    X1_next_ucb(i) = x_next_mean(1) + beta*sqrt(x_next_cov(1,1));
    
    if X1_next_true(i) >= X1_next_lcb(i) && X1_next_true(i) <= X1_next_ucb(i)
        Within_Bounds(i) = 1;
    else
        Within_Bounds(i) = 0;
    end
end
t = array2table([XU_test, X1_next_true, X1_next_lcb, X1_next_ucb, Within_Bounds],'VariableNames',{'x1_test' 'x2_test' 'u_test' 'x1_next_true' 'x1_next_lcb' 'x1_next_ucb' 'in_bounds'})
